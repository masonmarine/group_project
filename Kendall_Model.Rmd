...
# Summarize the counts by species and year (not separated by zone)
...
#create plot to show species counts by year
species_abundance <- ggplot(species_counts, aes(x = survey_year, 
                           y = total_count, 
                           color = classcode, 
                           group = classcode)) +
  geom_line() + 
  geom_point() +
  labs(title = "Species Counts by Year", 
       x = "Year", 
       y = "Total Count", 
       color = "Species") +
  theme_minimal() +
...
```{r}
#create figure that shows model results


```

The Negative Binomial regression model suggests that the abundance of Giant Kelp in study areas of Northern California kelp forests is significantly influenced by the presence of Sunflower Sea Stars (p < 0.001) and Purple Urchins (p < 0.001). Northern kelp crabs (p = 0.42) and zone by contrast are not significant predictors for giant kelp abundance. 