...
# Research Question: What environmental and biological factors (e.g., depth, size, species, site) are associated with different severities of disease in invertebrates in UCSC kelp forests?

# Hypotheses:
# Null Hypothesis (H₀): None of the predictors (depth, size, species, etc.) are significantly associated with disease severity.
# Alternative Hypothesis (H₁): At least one of the predictors significantly affects disease severity.
...
kelp_disease_data <- kelp_data %>% 
  ...
  mutate(disease_severity = factor(
   ...
   levels = c("Healthy", "Mild", "Severe")  # Simplify to a regular factor
  )) %>% 
...
disease_counts <- kelp_disease_data %>%
  ...
  labs(
    ...
    y = "Total Invertebrate Count",
    subtitle = "Distribution of disease severity by species",
    caption = "Data Source: Kelp Disease Dataset"
  ) +
  facet_wrap(~ species) +
  scale_y_continuous(trans = 'log10', labels = scales::comma) +  # Format y-axis labels with commas
...
size_counts <- kelp_disease_data %>%
  ...
  labs(
    ...
    x = "Size (cm)",
    y = "Total Count of Invertebrates",
...
qqline(kelp_disease_data_cooks$residuals, col = "red")  # Add a Q-Q line
...
# 4. Residuals vs Leverage Plot (using hat-values)
